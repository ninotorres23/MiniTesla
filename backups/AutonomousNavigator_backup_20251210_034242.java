...
public class AutonomousNavigator implements RobotController {
    ...
    private static final float BASE_SPEED = 0.30f;     // base speed for straight driving
    private static final float LINE_THRESHOLD = 0.30f; // lower threshold to reacquire sooner
    private static final float NUDGE_SCALE = 0.5f;    // gentler nudge to prevent overcorrection
    private static final float PIVOT_SPEED = 0.15f;    // stronger pivot to sweep line
    private static final int SEARCH_TIMEOUT_TICKS = 300; // ~6.0s at 20ms per tick
    private static final float MIN_FWD = 0.12f;       // allow tighter inside wheel during arcs
    private static final float TURN_ARC_SCALE = 0.10f; // minimum arc scale; dynamic during turn
    private static final int TURN_TICKS_MAX = 15;     // max ticks to pivot during a corner (~1.8s)
    private static final int SNAP_ON_TURN_TICKS = 0;   // snap heading immediately on turn
    private static final int SNAP_FORWARD_TICKS = 18;  // longer forward duration after snap
    private static final int POST_TURN_HOLD_TICKS = 20; // reduced hold time after turn
            ...
            case TURNING_LEFT:
                if (mid > LINE_THRESHOLD) {
                    ...
                    if (snapForwardTicks < SNAP_FORWARD_TICKS) {
                        ...
                        // Interleave short pivot pulses with arcs to sweep the line
                        if ((turnTicks % 12) < 3) {
                            car.setMotorSpeeds(-PIVOT_SPEED, PIVOT_SPEED);
                            System.out.println("TurningLeft: pivot pulse");
                        } else {
                            float leftSpeed = Math.max(MIN_FWD, BASE_SPEED * TURN_ARC_SCALE);
                            float rightSpeed = Math.max(MIN_FWD, BASE_SPEED);
                            car.setMotorSpeeds(leftSpeed, rightSpeed);
                            System.out.println(String.format("TurningLeft: arc forward (L:%.2f, R:%.2f)", leftSpeed, rightSpeed));
                        }
                    }
                    turnTicks++;
                }
            ...
            case TURNING_RIGHT:
                if (mid > LINE_THRESHOLD) {
                    ...
                    if (snapForwardTicks < SNAP_FORWARD_TICKS) {
                        ...
                        if ((turnTicks % 12) < 3) {
                            car.setMotorSpeeds(PIVOT_SPEED, -PIVOT_SPEED);
                            System.out.println("TurningRight: pivot pulse");
                        } else {
                            float leftSpeed = Math.max(MIN_FWD, BASE_SPEED);
                            float rightSpeed = Math.max(MIN_FWD, BASE_SPEED * TURN_ARC_SCALE);
                            car.setMotorSpeeds(leftSpeed, rightSpeed);
                            System.out.println(String.format("TurningRight: arc forward (L:%.2f, R:%.2f)", leftSpeed, rightSpeed));
                        }
                    }
                    turnTicks++;
                }
